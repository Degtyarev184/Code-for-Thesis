function [Distance] = CS_TOF_SingleFreq2_Noise(OrgDis)
...
LightSpeed = 3e8;
Fbase = 1000; %KHz
F2 = Fbase; %KHz = 1MHz %First frequency of the signal
PD1 = (2*F2*2*pi*10^3*OrgDis)/LightSpeed; %1st Component Phase Difference
SampPerCyc = 60;
Fs = 10; %Sampling Frequency at 10 KHz
L = (SampPerCyc*(F2)/Fs)/2; %Length of the signal reduced 2 times for faster calculation
LoopNum = SampPerCyc*(F2)/1000; %Signal repeat times number
...
for i=1:L
    SigObj(i) = cos(2*pi*F2/(Fbase*SampPerCyc)*i)+1;
    SigRef(i) = cos(2*pi*F2/(Fbase*SampPerCyc)*i+PD1)+1;
end
...
Distance = (LightSpeed/(2*F2*10^3))*(PDS/(2*pi));
